...
let rules = {
 ...
 "Pos": [
    ["се помера"], ["се помера  надоле"], ["се помера налево"], ["се помера надесно"]
  ],

  "Adv_mode": [
   ["гласно"], ["нагло"], ["сигурно"], ["лако"], ["брзо"], ["несигурно"], ["чудно"], ["са свим срцем"], ["лудо"],
  ],

  "VI": [
   ["мирише"], ["лиже"], ["цвили"], ["чеше"], ["жваће"], ["даје шапу"], ["лаје"], ["дахће"], ["пије"],
  ]
};
...
function toggle(){
 ...
 if (tog === 0){
  loop(); 
  tog = 1;
  button.html("Pause Cuko Чуко");
 }else{
   noLoop();
   tog =0;
   button.html("Start Cuko Чуко");
 }
